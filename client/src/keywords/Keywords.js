...
function deleteAll(){
	for(let i = 0; i < toTest.length; i++){
		...
		requestD.open('DELETE', requestURL+'/keywords/delete/'+toTest[i], true);
...
function KeysTable() {
	let KTable = document.getElementById('keysTable');
		KTable.innerHTML = "";
		KTable.innerHTML += "<tr><th class='keyEntry'>Key:</th> <th class='keyEntry'>Group:</th></tr>";
	for(let i = 0; i < toTest.length; i++){
		KTable.innerHTML += "<tr>";
    ...
    return (
		<div className="mainBody">
			...
			<div className="entryButtons">
				...
				<button className="entryButtons" type="submit" onClick={() => this.addKey()}> Add Key </button>
				<button className="entryButtons" type="submit" onClick={() => this.delKey()}> Remove Key </button>
